...
import { useTopReadersReport } from '@/queries/reportsQueries';
import { TopReaderReport, DateRange } from '@/types';

export default function TopReadersPage() {
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [dateRange, setDateRange] = useState<DateRange | undefined>(undefined);

  const { data, isLoading, error, refetch } = useTopReadersReport(
  ...
  const columns: TableColumn<TopReaderReport>[] = [
    {
      key: 'employee',
      label: 'Lector',
      sortable: true,
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                Lectores más activos ordenados por cantidad de trabajos
              </p>
            </div>
          </div>
          <Button
      ...
      <ReportTable
        ...
        emptyMessage={
          !dateRange
            ? 'Selecciona un rango de fechas para ver el top de lectores'
